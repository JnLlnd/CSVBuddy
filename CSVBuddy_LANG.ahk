...
global lTab2HelpSelect := "Select Fields$To remove fields (columns) from the list, enter the name of fields you want to keep, in the order they actually appear in the list. Each field must be separated by the field delimiter ( ~3~ ). Click ""Select"" to process fields.`n`nTo create a new field reusing previous fields, add reuse specs in the format ""~1~~1~fields and format~2~~1~new field name~2~~2~"" using the open and close reuse delimiters ( ~1~ ~2~ , see ""Options"" tab); in ~1~fields and format~2~, enclose reused fields by enclosing their name between delimiters (example ""~1~FirstName~2~ ~1~Name~2~""). If the reuse specs include the separator character ( ~3~ ), the whole reuse specs must be enclosed by the encapsulator character ( ~4~ ).`n`nTo save the file, click on the last tab ""3) Save CSV File""."
