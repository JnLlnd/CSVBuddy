...
global lAppVersion := "2.1.9.1" ; major.minor.fixes *** check strAppVersionLong for BETA or PROD
...
global lTab2HelpSelect := "Select Fields$To remove fields (columns) from the list, enter the name of fields you want to keep, in the order they actually appear in the list. Each field must be separated by the field delimiter ( ~3~ ).`n`nTo create a new field reusing previous fields, add reuse specs in the format ""~1~~1~fields and format~2~~1~new field name~2~~2~"" using the open and close reuse delimiters ( ~1~ ~2~ , see ""Options"" tab); in ~1~fields and format~2~, reuse fields by enclosing their name between delimiters (example ""~1~FirstName~2~ ~1~Name~2~""). Reuse fields can be inserted anywhere in the list. When reusing fields, all fields in the existing header must be present in the list. If the reuse specs include the separator or encapsulator characters, the whole reuse specs must be enclosed by the encapsulator character ( ~4~ ).`n`nClick ""Select"" to process fields. To save the file, click on the last tab ""3) Save CSV File""."
...
global lTab2SelectFieldMissingInReuse := "Field name ""~1~"" missing in the ""Select fields:"" zone.`n`nWhen building a reuse field, all fields in the existing header must me present in the ""Select fields:"" zone."
...
global lTab6ScreenHeightCorrection := "Screen height correction:"
global lTab6ScreenWidthCorrection := "Screen width correction:"
